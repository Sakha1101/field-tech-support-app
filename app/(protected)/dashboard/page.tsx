...
export default async function DashboardPage() {
  const { technician } = await getDashboardMetrics();
  ...
  const firstName =
    technician.full_name && technician.full_name.trim().length > 0
      ? technician.full_name.trim().split(" ")[0]
      : "Technician";
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-600">
                View login activity and recent access details.
              </p>
            </div>
            <Link
              href="/activity"
              className="rounded-2xl bg-ink px-4 py-3 text-sm font-bold text-white"
            >
              My Login Activity
            </Link>
          </div>
